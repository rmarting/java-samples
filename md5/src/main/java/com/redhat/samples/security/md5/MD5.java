package com.redhat.samples.security.md5;
...
public class MD5 {
    ...
    public void md5final() {
        ...
        if (index < 56) {
            ...
            padLen = 120 - (int) this.lngByteCount;
        }
        update(pad, padLen);
        update(bytBits, 8);
    ...
    public void digestFileToHexStr(File file, String prefix) throws Exception {


        FileInputStream is = new FileInputStream(file);
        int read = 0;
       // String prev = "";
       // String finale = prefix;
        long CTE_UNIDAD_TRATAMIENTO = 64;
        int NUMERO=64;
        // byte[] buffer = new byte[CTE_UNIDAD_TRATAMIENTO];

        // PREFIJO BIZKAIKO FORU ALDUNDIA
        char chrPrefix[] = new char[64];
        chrPrefix = prefix.toCharArray();
        this.lngByteCount=64;
        transform(this.lngState, chrPrefix);
       
        /* ' ----------------------------------------------------------------------------------
        '   Descripcion del proceso:
        '
        '       El fichero hay que tratarlo de la siguiente forma:
        '           1. Hay que leer de 0 a n buffers de 64 bytes
        '              y llamar a la funcion MD5TRANSFORM.
        '           2. Hay que leer un ultimo buffer de 1 a 64 bytes
        '              y llamar a la funcion MD5FINAL.
        ' ----------------------------------------------------------------------------------*/

        long x_lngBytesDisponibles;
        long x_lngTamano = file.length();
        long x_lngVeces = x_lngTamano / CTE_UNIDAD_TRATAMIENTO;
        long x_lngBytesResto = x_lngTamano % CTE_UNIDAD_TRATAMIENTO;
        if (x_lngBytesResto > 0) {
            x_lngVeces = x_lngVeces + 1;
        }
        boolean x_booUltimoBuffer = false;

        System.out.println("digestFileToHexStr2" + "x_lngTamano"+x_lngTamano);
        System.out.println("digestFileToHexStr2" + "x_lngVeces"+x_lngVeces);
        System.out.println("digestFileToHexStr2" + "x_lngBytesResto"+x_lngBytesResto);

         for (int x_lngNdx=1; x_lngNdx<=x_lngVeces;x_lngNdx++)
         {            
                if (x_lngNdx == x_lngVeces)
                {
                    x_booUltimoBuffer = true;
                }
                x_lngBytesDisponibles = CTE_UNIDAD_TRATAMIENTO;
                if (x_booUltimoBuffer && x_lngBytesResto > 0)
                {
                   x_lngBytesDisponibles = x_lngBytesResto;
                  // NUMERO=new Long(x_lngBytesResto).intValue();
                }
                ...
                //bytBuffer= new String (buffer).toCharArray();
                String cadena = new String(buffer,"ISO-8859-1");
                bytBuffer= cadena.toCharArray();
                System.out.println(cadena);
               // System.out.println(bytBuffer);

                this.lngByteCount=this.lngByteCount+x_lngBytesDisponibles;

                if (x_booUltimoBuffer)
                {
                    md5final();                    
             }
                else
                {                   
                    transform(this.lngState, bytBuffer);
                }
